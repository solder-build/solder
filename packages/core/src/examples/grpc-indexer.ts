...
async function main() {
  ...
  const config: RustIndexerConfig = {
    mode: 'grpc',
    startBlock: 378658369, // Historical starting point
    grpcEndpoint,
    xToken,
  ...
  await indexer.onEvent({
    ...
    handler: async (event: any) => {
      ...
      console.log("  Symbol:", event.parsed.symbol);
      console.log("  Mint:", event.parsed.mint);
      console.log("  User:", event.parsed.user);
    },
  });
